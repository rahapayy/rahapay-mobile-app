  ...
  async (error: AxiosError) => {
    ...
    if (status === 401) {
      if (!isRefreshing) {
        isRefreshing = true;
        try {
          const refreshTokenFromStorage = (await getItem("REFRESH_TOKEN", true)) ?? "";
          const accessTokenFromStorage = (await getItem("ACCESS_TOKEN", true)) ?? "";
          ...
          const newRefreshToken = response.refreshToken || refreshTokenFromStorage;
